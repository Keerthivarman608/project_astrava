...
import { Home, Search, Compass, Film, MessageCircle, Heart, PlusSquare, User, Menu, Instagram, X, LogOut } from 'lucide-react';
...
export const Layout: FC<LayoutProps> = ({ children, activeView, setActiveView }) => {
    ...
    return (
        <div className="flex flex-col md:flex-row min-h-screen bg-black text-[#F5F5F5] font-sans selection:bg-white/30 truncate-x-hidden relative overflow-x-hidden">
            ...
            <nav className={`hidden md:flex flex-col ${isDrawerOpen ? 'w-[72px]' : 'w-[72px] lg:w-[244px]'} border-r border-[#262626] fixed h-full z-50 bg-black pt-2 pb-5 px-3 transition-all duration-300`}>
                ...
                <div className="flex flex-col gap-1 relative h-full">
                    ...
                    <NavItem 
                        ...
                        icon={
                            profile?.avatar ? (
                                <img src={profile.avatar} className={`w-6 h-6 rounded-full border ${activeView === 'profile' ? 'border-white' : 'border-transparent'}`} alt="p" />
                            ) : (
                                <User className={activeView === 'profile' ? 'fill-white' : ''} />
                            )
                        } 
                    ...
                    <div className="mt-auto flex flex-col gap-1">
                        <NavItem label="Logout" icon={<LogOut />} onClick={logout} collapsed={isDrawerOpen} />
            ...
            {/* Sliding Drawer */}
            <div className={`fixed top-0 bottom-0 left-[72px] w-[397px] bg-black border-r border-[#262626] z-40  rounded-r-2xl shadow-[4px_0_24px_rgba(0,0,0,0.5)] transition-transform duration-300 ease-in-out ${isDrawerOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                {activeDrawer === 'search' && (
            ...
            {/* Main Content Area */}
            <main className={`flex-1 ${isDrawerOpen ? 'md:ml-[72px]' : 'md:ml-[72px] lg:ml-[244px]'} w-full h-full pb-[50px] md:pb-0 min-h-screen bg-black relative transition-all duration-300 flex justify-center`}>

                {/* Mobile Top Header (Only on Home) */}
                {activeView === 'home' && (
                    <div className="md:hidden fixed top-0 w-full h-[60px] bg-black border-b border-[#262626] flex items-center justify-between px-4 z-50">
                ...
                <div className={`w-full max-w-[630px] flex justify-center h-full min-h-screen ${activeView === 'home' ? 'pt-[60px]' : ''} md:pt-0`}>
                    {children}
                </div>
                    ...
                    <div className="hidden lg:block w-[320px] pt-[30px] pl-16 pr-4 mt-4">
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-4 cursor-pointer">
                                <img src={profile?.avatar || "https://i.pravatar.cc/150?u=developer_alex"} alt="You" className="w-[44px] h-[44px] rounded-full object-cover border border-[#262626]" />
                                <div className="flex flex-col">
                                    <span className="font-semibold text-sm text-[#F5F5F5]">{profile?.username || 'user'}</span>
                                    <span className="text-[#a8a8a8] text-[13px]">{profile?.fullName || 'Full Name'}</span>
                                </div>
                            </div>
